...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        ...
        let y = "big tipper" // replace with a string
        let z = 9.787 // replace with anything else!
        x |> should be ofType<int>
        y |> should be ofType<string>

    [<Test>]
    let ``09 A wildcard pattern will match anything`` () =
        let _ = 80 // replace with an integer
        let _ = "culture" // replace with a string
        let _ = 'b' // replace with anything else!
        ()
