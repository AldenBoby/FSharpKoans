    ...
    *)
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec reverse xs out = 
                    match xs with 
                        | [] -> out
                        | _ :: rest ->reverse rest (xs.Head::out)
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec mult xs out = 
                    match xs with 
                        | [] -> List.rev out
                        | _ :: rest ->mult rest (xs.Head*2::out)
            mult xs []// write a function which doubles each element
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec appf xs out = 
                    match xs with 
                        | [] -> List.rev out
                        | _ :: rest ->appf rest (f (xs.Head)::out)
            appf xs []// write a map which applies f to each element
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
             let rec fil xs out = 
                    match xs with 
                        | [] -> List.rev out
                        | _ :: rest ->match f(xs.Head) with 
                                       | true ->fil rest (xs.Head::out)
                                       | false -> fil rest (out)
             fil xs [] // write a function which filters based on the specified criteria
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            let rec odd xs out = 
                    match xs with 
                        | [] -> List.rev out
                        | _ :: rest ->match xs.Head%2=0 with 
                                       | false ->odd rest (xs.Head::out)
                                       | true -> odd rest (out) 
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let rec fold initialState xs =
                    match xs with 
                        | [] -> initialState
                        | _::rest -> let element = xs.Head
                                     let NextState = initialState + element
                                     fold NextState rest
    ...
    let ``15 A fold which multiplies a list`` () =
        let rec fold initialState xs =
                    match xs with 
                        ...
                        | _::rest -> let element = xs.Head
                                     let NextState = initialState * element
                                     fold NextState rest // write a function to multiply the elements of a list
    ...
    let ``16 Folding, the hard way`` () =
        let rec fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            ...
            |[] -> initialState
            |h::t -> let element = h 
                     let NextState = f initialState element
                     fold f NextState t   // write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let rec exists (f : 'a -> bool) (xs : 'a list) : bool =
            ...
            |h::t -> match f h with
                     | true -> true 
                     | false -> exists f t // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec makenew list newlist1 newlist2 = 
                match list with
                |[] -> (List.rev (newlist1),List.rev (newlist2))
                |h::t -> match f (h) with
                         |true -> let newlist1 = h::newlist1
                                  makenew t newlist1 newlist2
                         |false -> let newlist2 = h::newlist2
                                   makenew t newlist1 newlist2
            makenew xs [] [] // Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec doit count listi = 
                match count = n with
                | true -> List.rev (listi)
                | _ -> let valu = f count
                       let listi = valu::listi
                       doit (count+1) listi
        ...
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec pick list = 
                ...
                | h::t -> match p h with
                          | None -> pick t
                          | _ -> p h
            pick xs// Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
